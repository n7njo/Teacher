...
const blockCompositionSchema = Joi.object({
  section: Joi.string()
    .valid("introduction", "content", "practice", "assessment", "closure")
    .required(),
...
router.post("/lessons", async (req: any, res) => {
  try {
    ...
    const { topicId, name, slug, description, lessonType, blockComposition } =
      value;
...
router.get("/progress/:userId/lesson/:lessonId", async (req: any, res) => {
  try {
    ...
    const completedBlocks = result.rows.filter(
      (row: any) => row.status === "completed",
    ).length;
    const totalTime = result.rows.reduce(
...
router.post("/progress", async (req: any, res) => {
  try {
    ...
    if (!userId || !lessonId || !blockId) {
      return res
        .status(400)
        .json({ error: "userId, lessonId, and blockId are required" });
    }
